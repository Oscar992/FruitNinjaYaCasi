	...
	void Update () 
    {
        ...
        foreach (var body in data)
        {
            ...
            if (body.IsTracked)
			{
				...
				HandLeft.transform.position = Vector3.Lerp(HandLeft.transform.position,new Vector3(pos.X*scale, pos.Y*scale-yScale, -1*pos.Z*scale+30)-transform.position,Time.deltaTime*smoothMovement);

				pos = body.Joints[JointType.HandRight].Position;
				HandRight.transform.position = Vector3.Lerp(HandRight.transform.position,new Vector3(pos.X*scale, pos.Y*scale-yScale, -1*pos.Z*scale+30)-transform.position,Time.deltaTime*smoothMovement);

				pos = body.Joints[JointType.Head].Position;
				Head.transform.position = Vector3.Lerp(Head.transform.position,new Vector3(pos.X*scale, pos.Y*scale-yScale, -1*pos.Z*scale+30)-transform.position,Time.deltaTime*smoothMovement);
